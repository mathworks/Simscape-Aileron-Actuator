...
tune_bpth = [mdl '/Actuator/Hydraulic/Pressure Source'];
...
supplyP_curr = getparam(tg,'','hydr_supply_pressure');
disp(['Motor Bearing Friction (current) = ' num2str(supplyP_curr)]);
setparam(tg,'','hydr_supply_pressure',supplyP_curr*0.7)
disp(['Motor Bearing Friction (new) = ' num2str(getparam(tg,'','hydr_supply_pressure'))]);
...
%% Plot results of blocked line test
figure(1)
hold on
...
%save(['sm_aileron_actuator_slrt_hydr_res_' datestr(now,'yyddmm_HHMM')],'t_slrt1','y_slrt1','t_slrt2','y_slrt2','t_ref','y_ref','t_fs','y_fs');
